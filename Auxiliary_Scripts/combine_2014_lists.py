...
for pointing in ['minus_two', 'minus_one', 'zenith', 'plus_one', 'plus_two']:
    ...
    for page_ind in [1, 2, 3]:
        path = '%s/2014_EoR_High_pointing_%s_obs_page_%i.txt' % (rootdir, pointing, page_ind)
    ...
    make_obsfile(pointing_list, '%s/2014_EoR_High_Pointing_%s_obs.txt' % (rootdir, pointing))

make_obsfile(master_list, '%s/2014_EoR_High_Master_obs.txt' % rootdir)
