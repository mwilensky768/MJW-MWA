...
if not os.path.exists('%s/arrs/%s_None_INS_data.npy' % (outpath, obs)):
...
plot_lib.image_plot(fig, ax, ins.data[:, 0, :, 0], aspect=ss.UV.Nfreqs / ss.UV.Ntimes,
                    freq_array=ins.freq_array[0], cbar_label='Amplitude (UNCALIB)')
fig.savefig('%s/%s_INS.png' % (outpath, obs))
plt.close(fig)
fig, ax = plt.subplots(figsize=(8, 9))
plot_lib.image_plot(fig, ax, ins.data_ms[:, 0, :, 0], aspect=ss.UV.Nfreqs / ss.UV.Ntimes,
                    freq_array=ins.freq_array[0], cbar_label='Amplitude (UNCALIB)',
                    cmap=cm.coolwarm)
