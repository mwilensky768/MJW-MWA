...
if __name__ == "__main__":
    ...
    for shape in args.shapes:
        ...
        ax.set_title('%s Occupancy' % shape)
        fig.colorbar(cax, ax=ax, extend='min')
        fig.savefig('%s/%s_occ_scatter.pdf' % (args.outdir, shape))
    fig, ax = plt.subplots()
    occ_arr = np.mean([occ_dict[shape] for shape in occ_dict], axis=0)
    lim = np.amin(occ_arr[occ_arr > 0])
    print(len(occ_arr[occ_arr > 0]))
    print(np.amax(occ_arr[occ_arr > 0]))
    cax = ax.scatter(lsts, times, c=occ_arr, cmap=cmap, s=8, vmin=lim,
                     vmax=0.4)
    ax.set_xlabel('LST (hours)')
    ax.set_ylabel('JD (days)')
    ax.set_title('Total TV Occupancy')
    fig.colorbar(cax, ax=ax, extend='min')
    fig.savefig('%s/%s_occ_scatter.pdf' % (args.outdir, 'TV'))
